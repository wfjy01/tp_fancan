...
class HomeQiuGouTable extends Table
{
    // 设置当前模型对应的完整数据表名称
    protected $name = 'Home_QiuGou';
    //没有使用id作为主键名
    protected $pk = 'id';
    // 设置当前模型的数据库连接
    ...
    {
        try{
            $entityCount = $this->getReadQuery("select Count(id) as num  from  $this->name where Siteid = $this->site_id And isdel = 0 And IsSite = 0 And UserName = '".session('username')."' And DateDiff(day,UpTime,getdate())=0 ");
            return $entityCount;
        }catch (DbException $ex){
            return json($ex->getMessage());
        }
